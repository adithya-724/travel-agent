...
def hotel_agent(chat_history, verbose):
    ...
    task = Task(
        ...
        ''',
        expected_output="""
        JSON output which has the name of the hotel price of the hotel,ratings,room,link to the booking,type of hotel,check-in,check-out.
    ...
    crew = Crew(agents=[hotel_search_agent], tasks=[task], verbose=verbose)
    result = crew.kickoff()
